...
class ApiResponse(BaseModel, Generic[T]):
    ...
    @classmethod
    def success_page(cls, current: int, size: int, total: int, records: List[Any]) -> "ApiResponse[PageResult]":
        """分页成功响应"""
        page_info = PageResult(current=current, size=size,
...
if __name__ == "__main__":
    response_none = ApiResponse[None]()  # type: ignore
    print(response_none.model_dump_json(indent=2))

    response_object = ApiResponse.success({"id": 1, "name": "张三"})
    print(response_object.model_dump_json(indent=2))

    response_page = ApiResponse.success_page(
        current=1,
        size=20,
        total=36,
    ...
    response_error = ApiResponse.error(400, "参数校验失败：用户名不能为空")
